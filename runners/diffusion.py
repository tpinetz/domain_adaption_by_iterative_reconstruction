...
from functions.learning_rate_schedulers import WarmupCosineSchedule
...
def sample_gamma(model, device, dim, theta_0=1):
    seq = [i for i in range(1000)] #THIS WAS CHANGED FROM 1000
    b = torch.linspace(0.001, 0.02, steps=1000) #THIS WAS CHANGED FROM 1000
    a = (1 - b).cumprod(dim=0)
    k = (b / a ) / theta_0**2
    theta = (a.sqrt() * theta_0)
    k_bar = k.cumsum(dim=0)
    concentration = torch.ones((1, 1, dim, dim)).to(device) * k_bar[-1]
...
def poisson_steps(x, seq, model, b, **kwargs):
    with torch.no_grad():
        n = x.size(0)
        seq_next = [-1] + list(seq[:-1])
        xs = [x]
        ...
        for i, j in zip(reversed(seq), reversed(seq_next)):
            ...
            rates = torch.ones(mean.size()).to(x.device)* var**2
            e = (torch.poisson(rates) - rates)
            sample = mean + mask * e
            xs.append(sample.to('cpu'))
    return xs, x0_preds


def sample_poisson(model, device):
    seq = [i for i in range(1000)]
    b = torch.linspace(0.0001, 0.02, steps=1000)
    rates = torch.ones(1, 3, 64, 64)
    x = (torch.poisson(rates) - rates).to(device)
    xs, x_0 = poisson_steps(x, seq, model, b.to(device))
    return x_0[-1][0]
...
class Diffusion(object):
    ...
    def train(self):
        ...
        tb_logger = tb.SummaryWriter(log_dir=self.config.tb_logger)
        ...
        scheduler = WarmupCosineSchedule(optimizer, warmup_steps=config.optim.warmup_steps, t_total=config.training.n_iters)
        ...
        states = torch.load("exp/logs/highres_largetheta_largebeta/ckpt.pth")
        model.load_state_dict(states[0])
        ...
        for epoch in range(start_epoch, self.config.training.n_epochs):
            ...
            for i, (x, y) in enumerate(train_loader):
                ...
                # antithetic sampling
                    ...
                    if step % 1000 == 999:
                        img = sample_poisson(model, self.device)
                        tb_logger.add_image("Image_poisson", img, step, dataformats="CHW")
                    ...
                    if (step % 500 == 499 and step < 2000) or (step % 2000 == 1999 and step >= 2000):
                        img = sample_gamma(model, self.device, x.size(-1), theta_0=self.config.data.theta_0)
                        if Path("shift_image").is_file():
                            img = (1 + img)*0.5
                ...
                loss = loss / self.config.training.grad_accumulate
                if loss_ema == 0.0:
                    loss_ema = loss.item()

                tb_logger.add_scalar("loss", loss, global_step=step)
                loss_ema = loss_ema * ema_rate + loss.item() * (1 - ema_rate)
                logging.info(
                    f"step: {step}, loss: {loss.item():.2f}, loss_ema: {loss_ema:.2f}, lr {scheduler.get_last_lr()[0]:.2e}, data time: {data_time / (i+1):.3f}"
                )
                loss.backward()

                if (step + 1) % self.config.training.grad_accumulate == 0:
                    if self.config.training.grad_accumulate > 1:
                        logging.info(
                            f"optimizer step: {step // self.config.training.grad_accumulate}, loss: {loss.item()}, lr {scheduler.get_last_lr()[0]:.2e}, data time: {data_time / (i+1)}"
                        )
    ...
    def sample_image(self, x, model, last=True):
        ...
        if self.args.sample_type == "generalized":
            ...
            from functions.denoising import generalized_steps, generalized_gamma, generalized_concat, generate_noise
            ...
            elif self.config.data.noise_type:
                theta_0 = self.config.data.theta_0
